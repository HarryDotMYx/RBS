component hint="cfWheels ShortCodes Plugin" output="false" mixin="global"
{
	...
	public function init() {
		// custom hotfix train based on 0.2
		this.version = "1.2,2.5.0,3.0.0";
...
/********************* public ****************************/
	...
	public string function processShortcodes(string content=""){
		var result="";
		if (!len(content)) return content;
		if(!structKeyExists(application, "shortcodes") || structIsEmpty(application.shortcodes)){
			return content;
		}
...
/********************* internal ****************************/
	...
	public string function $sc_processTag(required array parts){
		var p2 = (arrayLen(parts) GTE 2 ? parts[2] : "");
		var p3 = (arrayLen(parts) GTE 3 ? parts[3] : "");
		var p4 = (arrayLen(parts) GTE 4 ? parts[4] : "");
		var p6 = (arrayLen(parts) GTE 6 ? parts[6] : "");
		var p7 = (arrayLen(parts) GTE 7 ? parts[7] : "");

		if(p2 EQ '[' AND p7 EQ ']'){
			return (arrayLen(parts) GTE 1 ? mid(parts[1], 2, len(parts[1]) - 2) : "");
		}
		return p2 & $sc_invokeTag(p3, $sc_parseAttributes(p4), p6) & p7;
	}

	/**
	 * @hint private invoke tag
	 */
	public string function $sc_invokeTag(required string tag, required struct attributes, string content=""){
		var o = "";
		try {
			if(
				structKeyExists(application, "shortcodes")
				&& structKeyExists(application.shortcodes, tag)
				&& isCustomFunction(application.shortcodes[tag])
			){
				o = application.shortcodes[tag];
				return o(attributes, content, tag);
			}
		} catch(any e){
			writeLog(file="rbs-shortcodes", type="error", text="$sc_invokeTag failed for tag=" & tag & ": " & serializeJSON(e));
			return "";
		}
		return "";
	}
