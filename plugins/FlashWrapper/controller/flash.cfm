...
<cffunction name="flashMessages" mixin="controller" returntype="string" access="public" output="false" hint="Displays a marked-up listing of messages that exists in the Flash."
	...
	<cfscript>
		...
		for (loc.i=1; loc.i <= loc.iEnd; loc.i++)
		{
			...
			loc.lowerCaseDynamicClassValues = false;
			if (StructKeyExists(arguments, "lowerCaseDynamicClassValues"))
			{
				if (IsBoolean(arguments.lowerCaseDynamicClassValues))
					loc.lowerCaseDynamicClassValues = arguments.lowerCaseDynamicClassValues;
				else if (IsSimpleValue(arguments.lowerCaseDynamicClassValues) && Len(Trim(arguments.lowerCaseDynamicClassValues)))
					loc.lowerCaseDynamicClassValues = ListFindNoCase("true,1,yes,on", Trim(arguments.lowerCaseDynamicClassValues)) > 0;
			}
			if (loc.lowerCaseDynamicClassValues)
